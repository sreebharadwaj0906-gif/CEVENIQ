...
`}],monorepo:{templateDir:"next-monorepo",init:async e=>{let t=T__default.resolve(e.projectPath,"packages/ui"),r=T__default.resolve(e.projectPath,"apps/web"),n=T__default.resolve(t,"components.json"),o=await L.readJson(n);e.registryBaseConfig&&(o=ho(o,e.registryBaseConfig)),o.tailwind.baseColor="neutral",e.rtl&&(o.rtl=true),await L.writeJson(n,o,{spaces:2});let i=T__default.resolve(r,"components.json"),s=await L.readJson(i);e.registryBaseConfig&&(s=ho(s,e.registryBaseConfig)),s.tailwind.baseColor="neutral",e.rtl&&(s.rtl=true),await L.writeJson(i,s,{spaces:2});let l=await Y(t,c.parse(o)),{config:a}=await W(e.components,l,{silent:true});await fe(e.components,a,{overwrite:true,silent:e.silent,isNewProject:true,skipFonts:true});let f=await Y(r,c.parse(s)),m=await ia$1(e.components,ca$1(a));if(m?.fonts?.length){let u={};for(let h of m.fonts)u[h.font.variable]=`var(${h.font.variable})`;await zt({theme:u},l,{silent:e.silent,overwriteCssVars:false,tailwindVersion:"v4"}),await ut(m.fonts,f,{silent:e.silent});}let p=l.iconLibrary;if(p&&p in a$3){let u=[...a$3[p].packages];await _(u,[],l,{silent:true}),await _(u,[],f,{silent:true});}return f},files:[{type:"registry:page",path:"app/page.tsx",target:"app/page.tsx",content:yo`import { ComponentExample } from "@/components/component-example";
...
`)}function Va(e,t){let r=[];r.push(yt(t)),r.push(dim("\u2502"));let n=e.files.slice(0,Ce);if(n.length===0&&!e.css)r.push(`${dim("\u2502")} ${dim("No changes.")}`),r.push(dim("\u2502"));else {for(let s of n)Bo(s,r);e.files.length>Ce&&r.push(dim("\u2502"));}let o=e.files.length;return o>Ce&&r.push(`  ${dim(`Showing ${Ce} of ${o} files. Use --diff <path> to view a specific file.`)}`),r.push(`${dim("\u2514")} ${dim("Run without --dry-run to apply.")}`),r.join(`
...
`)}function Bo(e,t){if(t.push(`${dim("\u251C")} ${bold(e.path)} ${dim("(")}${ht(e.action)}${dim(")")}`),e.action==="skip")t.push(`${dim("\u2502")} ${dim("No changes.")}`);else if(e.action==="create")Ve(t,e.content.split(`
`),r=>green(`+${r}`));else {let r=zo(e.existingContent,e.content,e.path);Ve(t,r);}t.push(dim("\u2502"));}function La(e,t,r){let n=[];n.push(yt(t)),n.push(dim("\u2502"));let o=Wo(e.files,r),i=e.css&&Uo(e.css.path,r);if(o.length===0&&!i)n.push(`${dim("\u2502")} ${yellow(`No file matching "${r}" found.`)}`),n.push(dim("\u2502"));else {for(let s of o){let l=s.content.split(`
